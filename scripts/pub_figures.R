...
save_fig <- function(p, name, w, h) {
  ...
  ggsave(paste0("figures/pub/", name, ".png"), p, width = w, height = h, dpi = 300)
  ggsave(paste0("figures/pub/", name, ".pdf"), p, width = w, height = h)
  message("Saved ", name)
}
...
message("Figure 1 ...")
try_fig({
  a1b <- read.csv("results/A1b_fixed_effects.csv") |>
...
try_fig({
  ...
  d3 <- bind_rows(
    get_cxlu("results/A2_fixed_effects.csv",  "colour_predictor", "A2 (colour x land use only)"),
    get_cxlu("results/A2c_fixed_effects.csv", "model",            "A2c (+ biome + phylogeny)")) |>
...
try_fig({
  ...
  c4 <- bind_rows(read_ct("results/A2c_fixed_effects.csv","A2c (relative abundance)"),
                  read_ct("results/A2d_fixed_effects.csv","A2d (abundance change)")) |>
