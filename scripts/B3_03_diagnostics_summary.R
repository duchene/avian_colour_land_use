...
print(diag_summary)
write_csv(diag_summary, "results/B3_convergence.csv")
...
write_csv(r2_summary, "results/B3_r2_summary.csv")
cat("\nBayesian R2:\n"); print(r2_summary)
...
write_csv(fixed_effects, "results/B3_fixed_effects.csv")
...
for (resp in names(fits_B3)) {
  ...
  ggsave(paste0("figures/B3_forest_", resp, ".png"), p, width = 7, height = 4, dpi = 150)
}

for (resp in names(fits_B3)) {
  p <- pp_check(fits_B3[[resp]], ndraws = 50, type = "dens_overlay") +
    ggtitle(paste("PP check (Dale):", resp)) + theme_minimal()
  ggsave(paste0("figures/B3_pp_check_", resp, ".png"), p, width = 7, height = 5, dpi = 150)
}
