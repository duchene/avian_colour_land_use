...
print(diag_summary)
write.csv(diag_summary, "results/A2b_convergence.csv", row.names = FALSE)
...
cat(rep("=", 60), "\n")
print(r2_summary)
write.csv(r2_summary, "results/A2b_r2_summary.csv", row.names = FALSE)
...
write.csv(fixed_effects, "results/A2b_fixed_effects.csv", row.names = FALSE)
...
for (cv in names(fits_A2b)) {
  ...
  ggsave(paste0("figures/A2b_pp_check_", cv, ".png"), p,
...
for (cv in names(fits_A2b)) {
  ...
  ggsave(paste0("figures/A2b_conditional_", cv, ".png"), p,
...
for (cv in names(fits_A2b)) {
  ...
  ggsave(paste0("figures/A2b_forest_", cv, ".png"), p,
...
for (cv in names(fits_A2b)) {
  ...
  ggsave(paste0("figures/A2b_lu_effects_", cv, ".png"), p,
         width = 7, height = 4, dpi = 150)
}
