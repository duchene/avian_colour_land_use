...
for (resp in responses) {
  checkpoint_file <- paste0("fits/B1b_fit_", resp, ".RData")
  if (file.exists(checkpoint_file)) {
    cat("\nLoading existing fit for:", resp, "\n")
    load(checkpoint_file); fits_B1b[[resp]] <- fit; next
  }
  ...
  pr  <- if (fam_name == "gaussian") priors_B1bb_gaussian else priors_B1b
...
save(fits_B1b, file = "fits/B1b_all_fits.RData")
cat("\nAll B1 models saved to fits/B1b_all_fits.RData\n")
