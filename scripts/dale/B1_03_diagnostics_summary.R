# ============================================================
# B1_03_diagnostics_summary.R
# Analysis B1: diagnostics, summaries, forest and pp-check plots.
# Mirrors A1_03. Outputs results/B1_{convergence,r2_summary,
# fixed_effects}.csv and figures/B1_forest_*, figures/B1_pp_check_*.
# ============================================================
...
diag_summary <- map_dfr(names(fits_B1), function(resp) {
  d <- check_convergence(fits_B1[[resp]])
...
print(diag_summary)
write_csv(diag_summary, "results/dale/B1_convergence.csv")

r2_summary <- map_dfr(names(fits_B1), function(resp) {
  r2 <- bayes_R2(fits_B1[[resp]])
  tibble(response = resp, R2_estimate = r2[1, "Estimate"], R2_error = r2[1, "Est.Error"],
         R2_lower = r2[1, "Q2.5"], R2_upper = r2[1, "Q97.5"])
})
write_csv(r2_summary, "results/dale/B1_r2_summary.csv")
cat("\nBayesian R2:\n"); print(r2_summary)

fixed_effects <- map_dfr(names(fits_B1), function(resp) {
  fe <- as.data.frame(fixef(fits_B1[[resp]]))
  fe$parameter <- rownames(fe); fe$response <- resp
  fe %>% select(response, parameter, everything())
})
write_csv(fixed_effects, "results/dale/B1_fixed_effects.csv")
cat("\nFixed effects saved to results/B1_fixed_effects.csv\n")

for (resp in names(fits_B1)) {
  df <- fixed_effects %>%
  ...
  ggsave(paste0("figures/dale/B1_forest_", resp, ".png"), p, width = 7, height = 4, dpi = 150)
}

for (resp in names(fits_B1)) {
  p <- pp_check(fits_B1[[resp]], ndraws = 50, type = "dens_overlay") +
    ggtitle(paste("PP check (Dale):", resp)) + theme_minimal()
  ggsave(paste0("figures/dale/B1_pp_check_", resp, ".png"), p, width = 7, height = 5, dpi = 150)
}
...
contrasts_tbl <- map_dfr(names(fits_B1), function(resp) {
  d  <- posterior::as_draws_matrix(fits_B1[[resp]])
...
write_csv(contrasts_tbl, "results/dale/B1_landuse_contrasts.csv")
print(as.data.frame(contrasts_tbl), digits = 3)

cat("\nB1 analysis complete. Results in results/ and figures/\n")
