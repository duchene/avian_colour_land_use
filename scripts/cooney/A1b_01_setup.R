# ============================================================
# A1b_01_setup.R
# Analysis 1b: A1 with land-use association scores built from
#              detections only, not from every raw row.
#
# A sensitivity check on the one input A1 is most exposed to. Everything
# else is identical to A1: same tree, same priors, same sampler, same
# responses, so the coefficients are directly comparable.
#
# One row per species. Colour is a species-level trait, so unlike
# A3b a phylogenetic random effect IS identifiable here: the
...
# ------------------------------------------------------------
# THE ONE DIFFERENCE FROM A1. A1 counts every row of the raw extract,
# and 84% of those rows are structural zeros, because PREDICTS stores a
# near-complete site by species matrix. Counting them measures how a
# study spread its sites across land uses, not where a bird was found:
# in VK1_2012__Otto 1 all 20 species score 116 primary / 27 secondary
# while their detections range from 13/2 to 53/9, and 908 species (53%)
# occur in a single study, so their score carries nothing about them.
# Here the proportions are built from detections only.
...
priors_A1b          <- model_priors("phylo_lognormal")
priors_A1bb_gaussian <- model_priors("phylo_gaussian")

dir.create("fits", showWarnings = FALSE)
save(spdat, responses, lu_predictors, priors_A1b, priors_A1bb_gaussian,
     response_families, nthreads, A, tree,
     file = "fits/A1b_model_setup.RData")
cat("\nA1 setup complete. Saved to fits/A1b_model_setup.RData\n")
