# ============================================================
# A2d_03_diagnostics_summary.R
# Convergence, fixed effects, variance components, Bayesian R2
# for A2d models. Outputs: results/A2d_{convergence,fixed_effects,
# variance_components,r2_summary}.csv
# ============================================================
...
load("fits/A2d_all_fits.RData")     # `fits` (named list, one per colour var)
load("fits/A2d_model_setup.RData")  # `dat_final` — to detect empty biome x land-use cells
dir.create("results/cooney", showWarnings = FALSE)

# Detect empty biome x land-use cells from the data and map them to the
# corresponding interaction parameter names (non-reference combos only).
# Such coefficients are prior-only and are flagged, not interpreted.
...
write_csv(conv, "results/cooney/A2d_convergence.csv")
cat("Convergence:\n"); print(conv)
...
write_csv(fixef_tbl, "results/cooney/A2d_fixed_effects.csv")
...
write_csv(varcomp, "results/cooney/A2d_variance_components.csv")
cat("\nVariance components:\n"); print(varcomp)
...
write_csv(r2, "results/cooney/A2d_r2_summary.csv")
cat("\nBayesian R2:\n"); print(r2)

cat("\nA2d diagnostics complete. Tables in results/.\n")
