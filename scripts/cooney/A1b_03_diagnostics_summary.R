...
# Analysis 1b: Diagnostics, summaries, and visualizations
# ============================================================
...
# Load data and fits
load("fits/A1b_model_setup.RData")
load("fits/A1b_all_fits.RData")
...
diag_summary <- map_dfr(names(fits_A1b), function(resp) {
  diag <- check_convergence(fits_A1b[[resp]])
  cat("\n===", resp, "===\n")
...
print(diag_summary)
write_csv(diag_summary, "results/cooney/A1b_convergence.csv")
...
r2_summary <- map_dfr(names(fits_A1b), function(resp) {
  cat("\n\n==========", resp, "==========\n")
  print(summary(fits_A1b[[resp]]))

  r2 <- bayes_R2(fits_A1b[[resp]])
  cat("\nBayes R2:\n")
  print(r2)
...
write_csv(r2_summary, "results/cooney/A1b_r2_summary.csv")

# ============================================================
# FIXED EFFECTS TABLE
# ============================================================

fixed_effects <- map_dfr(names(fits_A1b), function(resp) {
  fe <- as.data.frame(fixef(fits_A1b[[resp]]))
  fe$parameter <- rownames(fe)
  fe$response <- resp
  fe %>% select(response, parameter, everything())
})

write_csv(fixed_effects, "results/cooney/A1b_fixed_effects.csv")
cat("\nFixed effects saved to results/A1b_fixed_effects.csv\n")
...
for (resp in names(fits_A1b)) {
  ...
  ggsave(paste0("figures/cooney/A1b_forest_", resp, ".png"), p,
...
for (resp in names(fits_A1b)) {
  p <- pp_check(fits_A1b[[resp]], ndraws = 50, type = "dens_overlay") +
    ggtitle(paste("PP Check:", resp)) +
    theme_minimal()

  ggsave(paste0("figures/cooney/A1b_pp_check_", resp, ".png"), p,
...
contrasts_tbl <- map_dfr(names(fits_A1b), function(resp) {
  d  <- posterior::as_draws_matrix(fits_A1b[[resp]])
...
write_csv(contrasts_tbl, "results/cooney/A1b_landuse_contrasts.csv")
print(as.data.frame(contrasts_tbl), digits = 3)

cat("\nA1 analysis complete. Results in results/ and figures/\n")
