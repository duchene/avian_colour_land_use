...
# Extends A2b: adds Biome4 (main + 2-way interactions), the full
# PREDICTS random hierarchy, and a phylogenetic random effect.
# Study unit = canonical SS throughout (qualifying filter AND
# primary-veg baseline), vs A2b which used Reference.
# ============================================================
...
# ------------------------------------------------------------
# LOAD FULL RAW DATA + colour + Biome4
# ------------------------------------------------------------
...
# Biome4 — identical mapping to 00_data_preparation.R (keeps A2d
# consistent with A1/A1b/A2c, incl. Mediterranean -> Temperate Forest).
...
# Apply the standard collapse ONLY if Temperate Open x Plantation is sparse here.
...
dat_final <- dat_final %>% mutate(z_malecolcooney = zscore(malecolcooney),
...
# ------------------------------------------------------------
# FORMULAS (all 2-way; NO 3-way) + Gaussian priors (as A2b scale)
# ------------------------------------------------------------
...
priors_A2d <- c(prior(normal(0, 0.5), class = "Intercept"),
