# ============================================================
# A1_03_diagnostics_summary.R
# Analysis 1: Diagnostics, summaries, and visualizations
# ============================================================
...
diag_summary <- map_dfr(names(fits_A1), function(resp) {
  diag <- check_convergence(fits_A1[[resp]])
  cat("\n===", resp, "===\n")
...
print(diag_summary)
write_csv(diag_summary, "results/cooney/A1_convergence.csv")
...
r2_summary <- map_dfr(names(fits_A1), function(resp) {
  cat("\n\n==========", resp, "==========\n")
  print(summary(fits_A1[[resp]]))

  r2 <- bayes_R2(fits_A1[[resp]])
  cat("\nBayes R2:\n")
  print(r2)
...
write_csv(r2_summary, "results/cooney/A1_r2_summary.csv")

# ============================================================
# FIXED EFFECTS TABLE
# ============================================================

fixed_effects <- map_dfr(names(fits_A1), function(resp) {
  fe <- as.data.frame(fixef(fits_A1[[resp]]))
  fe$parameter <- rownames(fe)
  fe$response <- resp
  fe %>% select(response, parameter, everything())
})

write_csv(fixed_effects, "results/cooney/A1_fixed_effects.csv")
cat("\nFixed effects saved to results/A1_fixed_effects.csv\n")
...
for (resp in names(fits_A1)) {
  ...
  ggsave(paste0("figures/cooney/A1_forest_", resp, ".png"), p,
...
for (resp in names(fits_A1)) {
  p <- pp_check(fits_A1[[resp]], ndraws = 50, type = "dens_overlay") +
    ggtitle(paste("PP Check:", resp)) +
    theme_minimal()

  ggsave(paste0("figures/cooney/A1_pp_check_", resp, ".png"), p,
...
contrasts_tbl <- map_dfr(names(fits_A1), function(resp) {
  d  <- posterior::as_draws_matrix(fits_A1[[resp]])
...
write_csv(contrasts_tbl, "results/cooney/A1_landuse_contrasts.csv")
